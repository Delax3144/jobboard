...
// Константы для диапазона ползунка зарплаты
const MAX_SALARY_LIMIT = 50000;
...
export default function Jobs() {
  ...
  // Новые стейты для ручного ввода и ползунка зарплаты
  const [minSalary, setMinSalary] = useState<number>(0);
  const [maxSalary, setMaxSalary] = useState<number>(MAX_SALARY_LIMIT);

  // Стейты для мобильной модалки фильтров
  ...
  // Блокируем скролл основного экрана, когда открыта модалка фильтров
  useEffect(() => {
  ...
  const filteredJobs = jobs.filter(job => {
    ...
    // Новая логика фильтрации зарплаты по диапазону
  ...
  // Внутренний контент фильтров, чтобы не дублировать код для ПК и мобилки
    ...
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', paddingBottom: '20px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <button 
          ...
          style={{ background: 'none', border: 'none', color: '#10b981', fontSize: '13px', fontWeight: 700, cursor: 'pointer', padding: 0 }}
        >
          Clear All
        </button>
      </div>
      ...
      <div style={{ marginBottom: '35px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {FILTER_LOCATIONS.map(loc => (
            <label key={loc} style={{ display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer', fontSize: '14px', color: selectedLocations.includes(loc) ? '#fff' : '#888', fontWeight: selectedLocations.includes(loc) ? 600 : 400 }}>
              <div style={{ 
            ...
            return (
              <button 
                ...
                style={{ 
                  ...
                  borderRadius: '12px', padding: '8px 16px', fontSize: '13px', cursor: 'pointer', fontWeight: '600'
                }}
              >
                {lv}
              </button>
      ...
      {/* NEW: Salary Range Filter (Slider + Inputs) */}
      <div>
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '20px' }}>
          <div style={{ flex: 1 }}>
            ...
            <input 
              type="number" 
              value={minSalary} 
              onChange={(e) => setMinSalary(Math.max(0, parseInt(e.target.value) || 0))}
              style={{ width: '100%', background: '#000', border: '1px solid rgba(255,255,255,0.08)', padding: '10px', borderRadius: '10px', color: '#fff', fontSize: '13px', outline: 'none' }}
            />
          </div>
          <div style={{ flex: 1 }}>
            ...
            <input 
              type="number" 
              value={maxSalary} 
              onChange={(e) => setMaxSalary(Math.max(minSalary, parseInt(e.target.value) || 0))}
              style={{ width: '100%', background: '#000', border: '1px solid rgba(255,255,255,0.08)', padding: '10px', borderRadius: '10px', color: '#fff', fontSize: '13px', outline: 'none' }}
            />
          </div>
        </div>
        ...
        <div style={{ padding: '0 5px' }}>
          <input 
            type="range" 
            min="0" 
            max={MAX_SALARY_LIMIT} 
            step="1000"
            value={maxSalary > MAX_SALARY_LIMIT ? MAX_SALARY_LIMIT : maxSalary} 
    ...
    <div style={{ 
      ...
      <div className="container" style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 20px', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '50px', flexWrap: 'wrap', gap: '30px' }}>
          ...
          <div className="jobs-search-flex-group" style={{ display: 'flex', gap: '15px', width: '100%', maxWidth: '500px', alignItems: 'center' }}>
            ...
            <button 
              ...
              style={{ display: 'none', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', color: '#fff', padding: '16px', borderRadius: '16px', cursor: 'pointer', alignItems: 'center', justifyContent: 'center' }}
            >
              <Icons.Filter />
            </button>
          </div>
        ...
        <div className="jobs-layout" style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: '50px', alignItems: 'start' }}>
          ...
          {/* === SIDEBAR FILTERS (ДЛЯ ПК) === */}
          <aside className="jobs-sidebar" style={{ 
            ...
            {filteredJobs.map((job) => {
              ...
              return (
                <Link to={`/jobs/${job.id}`} key={job.id} style={{ textDecoration: 'none' }}>
                  <div className="job-card-flex" style={{ 
                  ...
                  >
                    ...
                    <div style={{ 
                      ...
                      boxShadow: '0 10px 20px rgba(0,0,0,0.4)', flexShrink: 0
                    }}>
                      {job.companyLogo ? (
                        <img 
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px', alignItems: 'center', marginBottom: '18px' }}>
                        ...
                        <span style={{ color: '#10b981', fontWeight: '800', fontSize: '15px' }}>
                          {job.salaryFrom.toLocaleString()} - {job.salaryTo.toLocaleString()} PLN
                        </span>
                      </div>
      ...
      <button 
        onClick={scrollToTop}
        style={{
          position: 'fixed', bottom: '30px', left: '30px', zIndex: 100, width: '45px', height: '45px', borderRadius: '50%',
          background: 'rgba(255, 255, 255, 0.05)', backdropFilter: 'blur(10px)', border: '1px solid rgba(255, 255, 255, 0.1)', color: '#fff',
          display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
          opacity: showTopBtn ? 1 : 0, visibility: showTopBtn ? 'visible' : 'hidden',
          transform: showTopBtn ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.3s cubic-bezier(0.16, 1, 0.3, 1)'
        }}
      >
      ...
      {/* === МОБИЛЬНАЯ ШТОРКА ФИЛЬТРОВ (ЧЕРЕЗ ПОРТАЛ) === */}
      {isFilterModalOpen && document.body && createPortal(
        <div className="premium-scroll" style={{ position: 'fixed', inset: 0, zIndex: 999999, display: 'flex', flexDirection: 'column', background: '#050505', padding: '30px 25px', overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '20px' }}>
            <button 
              onClick={() => setIsFilterModalOpen(false)}
              style={{ background: 'rgba(255,255,255,0.05)', border: 'none', color: '#fff', width: '40px', height: '40px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
            >
              <Icons.Close />
            </button>
          </div>
          ...
          <button 
            onClick={() => setIsFilterModalOpen(false)}
            style={{ width: '100%', background: 'linear-gradient(135deg, #10b981, #059669)', color: '#000', padding: '16px', borderRadius: '16px', fontWeight: 800, fontSize: '16px', border: 'none', marginTop: '40px', cursor: 'pointer', boxShadow: '0 10px 25px -5px rgba(16, 185, 129, 0.4)' }}
          >
