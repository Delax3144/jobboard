        ...
        try {
          ...
          res.data.forEach((app: any) => {
            ...
            if (lastUpdate > lastViewed || (!isEmployer && app.status === 'invited' && lastUpdate > lastViewed)) {
              hasUnread = true;
              const eventId = `${app.id}-${lastUpdate}`;

              // Если мы еще не уведомляли об этом событии
              if (!notifiedEventsRef.current.has(eventId)) {
                notifiedEventsRef.current.add(eventId);

                // Если юзер уже сидит в этом чате - не спамим
                if (location.pathname === `/messages/${app.id}`) return;

                const isMessage = app.messages?.length > 0 && app.messages[0].senderId !== user.id;
                const senderName = isEmployer 
                  ? `${app.candidate?.firstName} ${app.candidate?.lastName}` 
                  : app.job?.companyName;

                // 1. Проверяем настройку звука
                if ((user as any).soundEnabled !== false) {
                  playNotificationSound();
                }

                // 2. Проверяем настройку всплывающих пушей
                if ((user as any).toastsEnabled !== false) {
                  toast.custom((t) => (
                    ...
                    onClick={() => { toast.dismiss(t.id); navigate(`/messages/${app.id}`); }}
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: '13px', color: isMessage ? '#10b981' : '#3b82f6', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '4px' }}>
                          {isMessage ? 'New Message' : 'Status Update'}
                        </div>
                        <div style={{ color: '#fff', fontSize: '16px', fontWeight: 700 }}>
                          {isMessage ? `From ${senderName}` : `Action required for ${app.job.title}`}
                        </div>
                      </div>
                    </div>
                  ), { duration: 5000 });
  ...
  return (
    <>
      <style>{`
        ...
        .mobile-dropdown { position: fixed; top: 80px; left: 0; width: 100vw; height: calc(100vh - 80px); background: rgba(5, 5, 5, 0.95); backdrop-filter: blur(20px); z-index: 999; display: flex; flex-direction: column; padding: 40px 20px; gap: 20px; transform: translateY(-100%); opacity: 0; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); pointer-events: none; }
        .mobile-dropdown.open { transform: translateY(0); opacity: 1; pointer-events: all; }
        .mobile-link { font-size: 24px; font-weight: 800; color: #888; text-decoration: none; transition: color 0.2s; display: flex; align-items: center; }
        ...
        {user ? (
          <>
            <NavLink to="/profile" className={({isActive}) => `mobile-link ${isActive ? 'active' : ''}`} style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
              <div style={{ width: "40px", height: "40px", borderRadius: "50%", background: "#10b981", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: "bold", color: "#000", overflow: "hidden" }}>{user.avatarUrl ? <img src={user.avatarUrl?.startsWith('http') ? user.avatarUrl : `${apiUrl}${user.avatarUrl}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : user.email[0].toUpperCase()}</div>
              My Profile
            </NavLink>
            <button onClick={logout} style={{ background: 'transparent', border: 'none', color: '#ff4b4b', fontSize: '24px', fontWeight: 800, textAlign: 'left', padding: 0, marginTop: '10px', cursor: 'pointer' }}>Logout</button>
          </>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <NavLink to="/login" className="mobile-link">Log in</NavLink>
            <NavLink to="/register" style={{ background: '#10b981', color: '#000', padding: '16px', borderRadius: '16px', textDecoration: 'none', fontWeight: 800, fontSize: '18px', textAlign: 'center' }}>Sign Up Free</NavLink>
          </div>
        )}
      </div>
    </>
